...
| 3 | Estimate camera pose + trajectory | §4.3–4.5 |
...
The 2025–26 headline monocular SLAM systems (MASt3R-SLAM, VGGT-SLAM, SLAM3R, EC3R-SLAM) are
...
2. They need GPU inference; a GPU task on AWS costs ~20–40× a CPU task and complicates deployment.
...
weakness — a reviewer who knows the field will look for exactly this.
...
### 4.4 Tracking
Frame-to-**map** PnP (`solvePnPRansac`, iterative refinement) against the existing landmark set —
**not** frame-to-frame pose chaining. This matters for drift (§4.6, layer 2).
...
- **Drift panel:** toggle between the pre-optimization (odometry-only) and post-optimization
  trajectories in the same scene. This makes requirement #4 *visible* rather than merely claimed —
  the single highest-value UI element for a reviewer.
